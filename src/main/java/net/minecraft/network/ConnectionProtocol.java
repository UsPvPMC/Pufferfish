...
public enum ConnectionProtocol implements BundlerInfo.Provider {
    ...
    public int getPacketId(PacketFlow side, Packet<?> packet) {
        return this.flows.get(side).getId(packet.getClass());
    }

    @Override
    public BundlerInfo getBundlerInfo(PacketFlow side) {
        return this.flows.get(side).bundlerInfo();
    }
    ...
    static {
        for(ConnectionProtocol connectionProtocol : values()) {
            ...
            connectionProtocol.flows.forEach((side, handler) -> {
                handler.listAllPackets((packetClass) -> {
    ...
    static class PacketSet<T extends PacketListener> {
        ...
        public <P extends BundlePacket<T>> ConnectionProtocol.PacketSet<T> withBundlePacket(Class<P> bundlePacketType, Function<Iterable<Packet<T>>, P> bundleFunction) {
            if (this.bundlerInfo != BundlerInfo.EMPTY) {
                ...
                this.addPacket(BundleDelimiterPacket.class, (buf) -> {
                    return bundleDelimiterPacket;
                });
                this.bundlerInfo = BundlerInfo.createForPacket(bundlePacketType, bundleFunction, bundleDelimiterPacket);
                this.extraClasses.add(bundlePacketType);
                return this;
            }
        }

        public int getId(Class<?> packet) {
            return this.classToId.getInt(packet);
        }
